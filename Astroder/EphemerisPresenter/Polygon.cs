...
namespace EphemerisCalculator
{
    public class Polygon
    {
            ...
            try
            {
                if (FirstVertexDegree == sectorAngle)
                {
                    ...
                    if (nextCorner == 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                offset = Sector.OffsetFromDegree(Angle.Normalize(angle.Degrees - Vertices[Edges - 2].Degrees));
                    ...
                    {
                        offset = Sector.OffsetFromDegree(Angle.Normalize(Vertices[nextCorner].Degrees - angle.Degrees));
                ...
                else
                {
                    ...
                    else if (lowerCorner >= Edges - 1)
                    {
                        ...
                        {
                            offset = Sector.OffsetFromDegree(Angle.Normalize(FirstVertexDegree - angle.Degrees));
                            return last - (Edges - 1 + offset) * sideLength;
                        }
                        else
                        {
                    ...
                    {
                        offset = Sector.OffsetFromDegree(Angle.Normalize(angle.Degrees - Vertices[lowerCorner].Degrees));
        ...
        public bool IsOutstanding(Angle angle)
        {
            if (IsPolygon)
            {
                double degree = Angle.Normalize(angle.Degrees);
