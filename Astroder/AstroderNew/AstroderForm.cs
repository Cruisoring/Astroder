...
namespace Astroder
{
    public partial class AstroderForm : Form
    {
        ...
        private ToolStripComboBox comboFocused = new ToolStripComboBox();
        private ToolStripComboBox comboAnother = new ToolStripComboBox();
        ...
        //        String fileName = String.Format("{0}:{1}", Planet.Glyphs[id], PlanetEvent.PlanetEventCategorySymbols[category]);
        ...
        //            LineItem evtCurve = new LineItem(fileName, xValues.ToArray(), yValues.ToArray(), color, defaultEventSymbolType[category], 0);
        ...
        private void loadPlanets()
        {
            ...
            PlanetId first = CurrentEphemeris.Luminaries[0];
            planetsDict[first].Text = Planet.Glyphs[first].ToString();

            int index1 = comboFocused.SelectedIndex;
            int index2 = comboAnother.SelectedIndex;

            comboFocused.Items.Clear();
            comboAnother.Items.Clear();
            ...
            comboFocused.SelectedIndex = index1;
            comboAnother.SelectedIndex = index2;
        ...
        private Dictionary<PlanetId, ToolStripButton> planetsDict = new Dictionary<PlanetId, ToolStripButton>();
        ToolStripButton allPlanet = new ToolStripButton();
        ...
        private void getInterDayToolStripReady()
        {
            ...
            Font astroFont = new System.Drawing.Font("AstroSymbols", 9F); 
            foreach (PlanetId id in CurrentEphemeris.Luminaries)
            {
                ...
                planetsDict.Add(id, btn);
                if (id == PlanetId.SE_SUN)
                    planetsDict.Add(PlanetId.SE_EARTH, btn);
                else if (id == PlanetId.SE_EARTH)
                    planetsDict.Add(PlanetId.SE_SUN, btn);
            }

            allPlanet.AutoSize = false;
            allPlanet.Font = astroFont;
            allPlanet.Text = "All";
            allPlanet.CheckedChanged += new EventHandler(allPlanet_CheckedChanged);
            toolStrip1.Items.Add(allPlanet);
        ...
        private void synchPlanetOffsetStatus()
        {
            ...
            //bool isChecked = allPlanet.Checked;
        ...
        {
            if (planetsDict.ContainsKey(id))
            {
                return !planetsDict[id].Checked;
            }
            else
                return false;
        ...
        //        LineItem orbit = new LineItem(fileName, xValues.ToArray(), yValues.ToArray(), color, defaultEventSymbolType[category], 0);
