...
namespace AstroHelper
{
    ...
    public class MatchRules// : IComparable<MatchRules>
    {
        ...
        {
            get { return Utilities.UtcFromJulianDay(referedJulianUtc); }
            set { referedJulianUtc = Utilities.ToJulianDay(value); }
        }

        public DateTimeOffset Since
        {
            get { return Utilities.UtcFromJulianDay(startJulianUtc); }
        }

        public DateTimeOffset Until
        {
            get { return Utilities.UtcFromJulianDay(endJulianUtc); }
        }
                ...
                case SearchMode.WithinTheMonth:
                    {
                        ...
                        endJulianUtc = Utilities.ToJulianDay(firstOfMonth) + DefaultDayPeriod.TotalDays;
                        break;
                    }
                case SearchMode.WithinTheYear:
                    {
                        ...
                        endJulianUtc = Utilities.ToJulianDay(firstOfYear) + DefaultDayPeriod.TotalDays;
                        break;
                    }
    ...
    public class UtcDayPeriod : MatchRules, IComparable<UtcDayPeriod>
    {
        public UtcDayPeriod(DateTimeOffset time) : base(time.UtcDateTime.Date, SearchMode.WithinTheDay)
        {
        }

        #region IComparable<UtcDayPeriod> 成员

        public int CompareTo(UtcDayPeriod other)
        {
            return StartJulianUtc.CompareTo(other.StartJulianUtc);
        }
